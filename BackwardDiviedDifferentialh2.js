...
var dataInTable = []
var data = []
class BackwardDiviedDifferentialh2 extends Component
{ 
  constructor()
  {
    super();
...
/* ส่วนดึงข้อมูลจาก MongoDB */
    componentDidMount = async() => { 
        await api.getFunctionByName("BackwardDiviedDifferentialh2").then(db => {
        this.setState({
    ...
    onSubmit()
  {
    ...
    if(round === 1)
    {
       ...
       answercal = (((3*xcal[0]) - (4*xcal[1]) + xcal[2]) / (2*pow(h,2)))
    }
    else if(round === 2)
    {
       ...
       answercal = (((2*xcal[0]) - (5*xcal[1]) + (4*xcal[2]) - xcal[3]) / pow(h,3))
    }
    else if(round === 3)
    {
       ...
       answercal = (((5*xcal[0]) - (18*xcal[1]) + (24*xcal[2])  - (14*xcal[3]) + xcal[4]) / (2*pow(h,4)))
    }
    else if(round === 4)
    {
       ...
       scopecal.x = scope.x - (h*5)
       xcal.push(expr.evaluate(scopecal))
       
       answercal = (((3*xcal[0]) - (14*xcal[1]) + (26*xcal[2]) - (24*xcal[3]) + (11*xcal[4]) - (2*xcal[5])) / pow(h,4))
  ...
  createTable(xcal,xreal,exprreal) 
  {
    ...
    return(
        <React.Fragment>
        <div width={10000}>
            <h1 style={Textstyle} className="text-white"> Backward Divied Differential(h^2)</h1>
      <Form>
          <br/>
          <Form.Row>
...
export default BackwardDiviedDifferentialh2;
var Textstyle = {
  textAlign:'center',
