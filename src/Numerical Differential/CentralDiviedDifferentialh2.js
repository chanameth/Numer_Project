...
var data = []
var fxr = [] , fxl = []
class CentralDiviedDifferentialh extends Component
{ 
  constructor()
  {
    super();
...
componentDidMount = async() => { 
    await api.getFunctionByName("CentralDiviedDifferentialh2").then(db => {
    this.setState({
    ...
    onSubmit()
  {
    ...
    else if(round === 3)
    {
        ...
        scopecal.x = scope.x - (2*h)
       xcal.push(expr.evaluate(scopecal))

       answercal = ((xcal[0] - (2*xcal[1]) + (2*xcal[2])  - xcal[3]) / (2*pow(h,3)))
    }
    else if(round === 4)
    {
        scopecal.x = scope.x + (h*2)
  ...
  createTable(xcal,xreal,exprreal) 
  {
    ...
    return(
        <React.Fragment>
        <div width={10000}>
            <h1 style={Textstyle} className="text-white"> Central Divied Differential(h^2)</h1>
      <Form>
          <br/>
          <Form.Row>
...
export default CentralDiviedDifferentialh;
var Textstyle = {
  textAlign:'center',
