...
var dataInTable = []
var data = []
class ForwardDiviedDifferentialh2 extends Component
{ 
  constructor()
  {
    super();
...
/* ส่วนดึงข้อมูลจาก MongoDB */
    ...
    onSubmit()
  {
    ...
    { 
       ...
       scopecal.x = scope.x
       xcal.push(expr.evaluate(scopecal))
       answercal = (((-(xcal[0]) + (4*xcal[1])) -(3*xcal[2])) / (2*pow(h,2)))
    }
    else if(round === 2)
    {
      ...
      xcal.push(expr.evaluate(scopecal))
       ...
       answercal = (((-(xcal[0]) + (4*xcal[1])) -(5*xcal[2]) + (2*xcal[3])) / (pow(h,3)))
    }
    else if(round === 3)
    {
      ...
      xcal.push(expr.evaluate(scopecal))
       ...
       answercal = (((-(3*xcal[0]) + (14*xcal[1])) - (24*xcal[2]) + (18*xcal[3]) - (5*xcal[4])) / (2*pow(h,4)))
    }
    else if(round === 4)
    {
      ...
      xcal.push(expr.evaluate(scopecal))
       ...
       answercal = (((-(2*xcal[0]) + (11*xcal[1])) - (24*xcal[2]) + (26*xcal[3]) - (14*xcal[4]) + (3*xcal[5])) / (pow(h,5)))
...
export default ForwardDiviedDifferentialh2;
var Textstyle = {
  textAlign:'center',
