...
/* ส่วนดึงข้อมูลจาก MongoDB */
  ...
  onSubmit()
  {
    ...
    {
      if(this.funcChange(parseFloat(this.state.Xr)) * this.funcChange(parseFloat(this.state.Xl)) >= 0)
        {
        dataInTable=[]
        var sum = 0.0
        ...
        /* ทำทิ้งเปล่า 1 ครั้ง */
        inputy['xl'][n] = (parseFloat(xl)).toFixed(6)
        inputy['xr'][n] = (parseFloat(xr)).toFixed(6)
        xm = ((parseFloat(xl) + parseFloat(xr)) / 2).toFixed(6)
        inputy['xm'][n] = xm
        inputy['error'][n] = 1
        fxr[n] = this.funcChange(xr)
        fxl[n] = this.funcChange(xl)
        fxm[n] = this.funcChange(xm)
  ...
  createTable(xl, xr, xm, error) {
    ...
    return(
        <React.Fragment>
        <div width={10000}>
            <h1 style={Textstyle} className="text-white">Bisection</h1>
      <Form>
