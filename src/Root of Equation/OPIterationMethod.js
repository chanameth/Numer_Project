...
class OPIterationMethod extends Component
{ 
  constructor()
  {
    super();
  ...
  componentDidMount = async() => { 
    await api.getFunctionByName("One-Point-Iteration-Method").then(db => {
    this.setState({
  ...
  onSubmit()
  {
        ...
        console.log(simplify(this.state.function).toString())
        var inputy = []
        inputy['x'] = []
        inputy['error'] = []
        console.log(x)
        /* ทำทิ้งเปล่า 1 ครั้ง */
        inputy['x'][n] = x.toFixed(6)
        inputy['error'][n] = 1
         x = this.funcChange(x).toFixed(6)
         console.log("x ครั้งแรก" + " " + x)
        fx[n] = x
        inputy['x'][n+1] = x 
      /* console.log("Iteration"+n+" "+"xl:"+data['xl'][n]+" "+"xr:"+data['xr'][n]+" "+"xm:"+data['xm'][n]+" "+"error:"+data['error'][n])*/
        ...
        do
        {
          ...
          sum = this.funcError(x,fx[n]).toFixed(6)
  ...
  createTable(x,error) {
    ...
    return(
        <React.Fragment>
        <div width={10000}>
            <h1 style={Textstyle} className="text-white">Onpoint Iteration</h1>
      <Form>
          <Form.Group as={Row} controlId="functionOPIterationMethod">
          ...
          <Form.Group as={Row} controlId="VariableXOPIterationMethod">
                  <Form.Label column sm="2">
...
export default OPIterationMethod;
var Textstyle = {
  textAlign:'center',
