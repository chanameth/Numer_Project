...
/* ส่วนดึงข้อมูลจาก MongoDB */
  componentDidMount = async() => { 
    await api.getFunctionByName("CompositeTrapezoidal").then(db => {
    this.setState({
    ...
    onSubmit()
  {
    ...
    for( i = 1 ; i < n ; i++)
            {   
                ...
                xical +=parseFloat(cal[i-1])
                console.log(xical)
            }
    xcal.push(xical)
    scopecal.x = b
    xcal.push(evaluate(this.state.function, scopecal))
    answercal = (((b-a)/(n*2))*(xcal[0] +  (2*xcal[1]) + xcal[2])).toFixed(6)
    error = abs(((xreal - answercal)/xreal)).toFixed(6)

     this.setState({showTable:true})
     this.createTable(xcal,xreal,fxintegral,cal,n,error,a_h)
  }
  onReset ()
  {
  ...
  createTable(xcal,xreal,fxintegral,cal,n,error,a_h) 
  {
      dataInTable = []
    for (var i = 0; i < n+1 ; i++) {
        dataInTable.push({
    ...
    return(
        <React.Fragment>
        <div width={10000}>
            <h1 style={Textstyle} className="text-white"> CompositeTrapezaidel </h1>
      <Form>
          <br/>
          <Form.Row>
